...
export class AlertHandlerService {
  ...
  public raiseError(error: any | string) {
    ...
    let message: string = typeof error === 'string' ? error : (error.error && error.error.message ? error.error.message : error.message)
    if (message.startsWith('Http failure response for') && message.endsWith('0 Unknown Error')) {
